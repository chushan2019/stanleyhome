...
## 从网页复制文章粘贴进「正文」会发生什么（⚠️ 需粘贴两次）

2026-09 两起后台事故，均已修复并有真实键入级回归（`npm run qa:paste`，headful + 系统剪贴板 + 真 ⌘V）：

1. **整页崩溃**：Decap 富文本粘贴管线遇到网页里的懒加载占位图（无 src）、内联 SVG 等
   会产生畸形 image 节点 → 预览渲染解构 undefined 白屏（上游至 decap-cms 3.16.2 未修复）；
2. **假粘贴**：修复过程中曾用 execCommand 注入文本——字看得见但表单状态为空，
   点 Save/Publish 弹「Oops, you've missed a required field」。

现行方案（`admin/index.html` 内置防护层）：粘贴网页富文本时**拦下并转换**，把
HTML 转成规范 Markdown（图片语法 `![]()` 一律转为文字占位「（图：… URL）」）
写回系统剪贴板，屏幕弹出黑色提示条。

### 正确操作

1. 正文编辑器里 ⌘V 粘贴 → 出现提示「已拦截网页格式并转存为纯文本：请按 ⌘V 再粘贴一次」
2. **再按一次 ⌘V** → 纯文本落地，编辑器原生路径处理，状态可靠、不会崩溃
3. 需要真图片：用正文工具栏插入图片，或表单「封面图」上传（URL 已保留在占位文字里，可对照补图）
4. 想精确保留 Markdown 字符（`**`、`#` 等原样入库）：第二次粘贴前把编辑器右上角
   **Rich Text 开关切到 Markdown 模式**——富文本模式粘贴会重新序列化，个别强调标记可能丢失
5. 截图直接粘贴（剪贴板是图片文件）不受影响，走原上传管线；
   纯文本粘贴（不含图片语法）也不拦截、一次完成

回归：`npm run qa:paste`（第一次拦截+提示 → 第二次落地 → 发布落盘含正文 → 图片语法全形式转占位）。
