...
quotes:
  - test摘录
---
For the complete documentation index, see [llms.txt](https://learn.chatgpt.com/llms.txt). Markdown versions of documentation pages are available by appending `.md` to the page URL.[ChatGPT](https://learn.chatgpt.com/)Primary navigationDocsUse casesTrainingResourcesOverviewFeaturesConfigurationDevelopersSecurityAdministrationUse CasesResources
...
*   [Desktop app](https://learn.chatgpt.com/docs/linux/linux-app)
...
1. **Global scope:**
    ...
    In your Codex home directory (defaults to 
   ...
   `AGENTS.override.md`
    ...
    if it exists. Otherwise, Codex reads 
...
2. **Project scope:**
    ...
    Starting at the project root (typically the Git root), Codex walks down to your current working directory. If Codex cannot find a project root, it only checks the current directory. In each directory along the path, it checks for 
...
Keep rules concise, explain the behavior to flag and any safe path or exception, and reserve formatting and lint checks for CI. See [Customize what Codex reviews](https://learn.chatgpt.com/docs/third-party/github#customize-what-codex-reviews) for setup and rule-writing guidance.
...
* .agents.mdFallback file in root
* support/
...
* Run 
  ...
  `codex --ask-for-approval never "Summarize the current instructions."`
   ...
   from a repository root. Codex should echo guidance from global and project files in precedence order.
* Use 

  `codex --cd subdir --ask-for-approval never "Show which instruction files are active."`

   to confirm nested overrides replace broader rules.
* To audit which instruction files Codex loaded, opt into a plaintext TUI log with 
  ...
  , or inspect the most recent 

  `session-*.jsonl`

   file if you enabled session logging.
...
* **Nothing loads:**
   ...
   Verify you are in the intended repository and that 

  `codex status`

   reports the workspace root you expect. Ensure instruction files contain content; Codex ignores empty files.
* **Wrong guidance appears:**

   Look for an 

  `AGENTS.override.md`

   higher in the directory tree or under your Codex home. Rename or remove the override to fall back to the regular file.
* **Codex ignores fallback names:**

   Confirm you listed the names in 

  `project_doc_fallback_filenames`

   without typos, then restart Codex so the updated configuration takes effect.
* **Instructions truncated:**
  ...
  `project_doc_max_bytes`
   ...
   or split large files across nested directories to keep critical guidance intact.
* **Profile confusion:**
  ...
  `echo $CODEX_HOME`
   ...
   before launching Codex. A non-default value points Codex at a different home directory than the one you edited.
...
* Review 
  ...
  [Prompting Codex](https://learn.chatgpt.com/docs/prompting)
   ...
   for conversational patterns that pair well with persistent guidance.

## Docs agent

Loading docs agent...

Copy Page[NextSubagents](https://learn.chatgpt.com/docs/agent-configuration/subagents)Ask AI
